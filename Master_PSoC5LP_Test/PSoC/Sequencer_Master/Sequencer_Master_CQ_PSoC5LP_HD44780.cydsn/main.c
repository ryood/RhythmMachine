...
#if 0    
    if (status == SEQUENCER_TRANSFER_CMPLT)
    {
        // SEQUENCE1 SEQUENCE2 POT1 POT2
        LCD_printf(0, "%03d %03d %02d %02d   ",
            sequencerRdBuffer.sequence1,
            sequencerRdBuffer.sequence2,
...
uint32 writeSequencerBoard(void)
{
    ...
    {
        // UPDATE TRACK PLAY 
        LCD_printf(1, "%2d %1d %1d %2d",
            sequencerRdBuffer.update,
            sequencerRdBuffer.track,
            sequencerRdBuffer.play,
            sequencerWrBuffer[0]
        );
    }
    else
